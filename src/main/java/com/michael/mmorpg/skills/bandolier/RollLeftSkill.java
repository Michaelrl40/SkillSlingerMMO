...
public class RollLeftSkill extends Skill {
    private final double velocity;
    private final double height;
    private final double invulnFrames;
    private final int duration;

    public RollLeftSkill(ConfigurationSection config) {
        super(config);
    ...
    protected void performSkill(Player player) {
        ...
        // Calculate left direction relative to player's facing
        Vector direction = player.getLocation().getDirection().crossProduct(new Vector(0, 1, 0))
                .normalize().multiply(-1).multiply(velocity);
